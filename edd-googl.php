 ...
 * Description:     Automatically create a Goo.gl shortened url on your Easy Digital Downloads products
 * Version:         1.0.2
 * Author:          Tsunoa
...
if( !class_exists( 'EDD_Googl' ) ) {
    ...
    class EDD_Googl {
        ...
        private function includes() {
            ...
            require_once EDD_GOOGL_DIR . 'includes/widgets.php';
        }
        ...
        private function hooks() {
            ...
            add_action( 'edit_form_after_title', array( $this, 'shortlink_box' ) );

            // Updates download shortlink on save post
            add_action( 'save_post', array( $this, 'update_post_shortlink' ) );
        }
         ...
         */
        public function shortlink_box( $post ) {
            if( $post->post_type == 'download' || $post->post_type == 'edd_download_page' ) {
                $shortlink = $this->get_shortlink( $post->ID );
         ...
         */
        public function update_post_shortlink( $post_id = null ) {
            if( $post_id === null ) {
                $post_id = get_the_ID();
            }

            // Only creates a shortlink if post has published
            if( $post_id && get_post_status( $post_id ) === 'publish' ) {
                $permalink = get_permalink( $post_id );

                $shortlink = $this->shorten($permalink);

                update_post_meta( $post_id, 'edd_googl_url', $shortlink );
            }
        }
         ...
         */
        public function get_shortlink( $post_id = null ) {
            if( $post_id === null ) {
                $post_id = get_the_ID();
            }

            return get_post_meta( $post_id, 'edd_googl_url', true );
        }

        /**
         * Deprecated, backward compatibility
         *
         * @deprecated Use edd_googl()->get_shortlink() instead
         *
         * @access      public
         *
         * @since       1.0.0
        ...
        public function get_download_shortlink( $download_id ) {
            return $this->get_shortlink( $download_id );
        }
    }
} // End if class_exists check

/**
 * Shortcut for edd_googl()->get_shortlink()
 *
 * @since       1.0.0
 * @return      string The shortened url or an empty string
 */
function edd_googl_shortlink( $post_id = null ) {
    return edd_googl()->get_shortlink( $post_id );
}
